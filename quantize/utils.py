...
class MultiBlock(nn.Module):
    ...
    def forward(self,
        ...
        for block in self.block_list:
            hidden_states = block(
                hidden_states, attention_mask=attention_mask, position_ids=position_ids)[0]
...
class TruncateFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, threshold):
        ...
        truncated_tensor[truncated_tensor.abs(
        ) < threshold] = truncated_tensor[truncated_tensor.abs() < threshold].sign() * threshold
